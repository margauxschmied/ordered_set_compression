...
public class Huffman extends AbstractCompressor {
    private Map<Object, Object> charPrefixHashMap = new HashMap<>();
    ...
    public HuffmanData compress(List<Integer> toCompress) {
        ...
        for (int i = 0; i < toCompress.size(); i++) {
            ...
            res.add((String) charPrefixHashMap.get(e));
        }
    ...
    public List<Integer> decompress(List<String> toDecompress, List<String> tree) {
        Map<Object, Object> map=new HashMap<>();

        for(int i=0; i< tree.size(); i+=2){
            map.put(tree.get(i+1), Integer.valueOf(tree.get(i)));
        }


        return decompress(new HuffmanData(map, toDecompress));
    }

    public List<Integer> decompress(HuffmanData huffmanData) {
        List<String> toDecompress = huffmanData.getList();
        List<Integer> res = new ArrayList<>();
        Map<String, Integer> code = huffmanData.getCode();

        for (String val : toDecompress) {
            res.add(code.get(val));
